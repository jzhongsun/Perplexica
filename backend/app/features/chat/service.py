...
class ChatService:
        ...
        try:
            ...
            try:
                async for token in result.aiter():
                    ...
                    yield json.dumps(
                        StreamResponse(
                            type="message", data=token, messageId=ai_message_id
                        ).model_dump()
                    ) + "\n"
                ...
                yield json.dumps(
                    StreamResponse(
                        type="messageEnd", messageId=ai_message_id
                    ).model_dump()
                ) + "\n"
                ...
                yield json.dumps(
                    StreamResponse(
                        type="error", data=str(e), messageId=ai_message_id
                    ).model_dump()
                ) + "\n"
                raise
            ...
            raise HTTPException(
                status_code=500, detail=f"Error in chat stream: {str(e)}"
            )
    ...
    async def get_response(self, request: ChatRequest) -> NonStreamResponse:
        ...
        try:
            ...
            # Save assistant message
            await self._save_message(
                ai_message_id, request.message.chatId, "assistant", result.text, sources
            )

            return NonStreamResponse(message=result.text, sources=sources)
