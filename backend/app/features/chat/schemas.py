...
def create_message_part(data: Dict[str, Any]) -> UIMessagePart:
    ...
    if part_type == "text":
        ...
        return DataUIPart(type=part_type, id=data.get("id"), data=data["data"])
...
class ChatRequestOptions(BaseModel):
    optimizationMode: Optional[Literal["speed", "balanced", "quality"]] = Field(
        default=None, description="Optimization mode"
    )
    focusMode: Optional[str] = Field(default=None, description="Focus mode")
    system_instructions: Optional[str] = Field(
        default=None, description="System instructions"
    )

class ChatStreamRequest(BaseModel):
    id: Optional[str] = Field(default=None, description="Chat ID")
    messages: List[UIMessage] = Field(default_factory=list, description="List of messages")
    options: Optional[ChatRequestOptions] = Field(default=None)
    files: Optional[List[ChatFile]] = Field(default_factory=list, description="List of files")

class ChatRequest(BaseModel):
    """Chat request model."""

    chatId: Optional[str] = Field(
        default=None, description="Chat ID for continuing conversation"
    )
...
class ChatResponse(BaseModel):
    ...
    chatTitle: str = Field(..., description="Chat title")
...
class StreamResponse(BaseModel):
    type: Literal["message", "sources", "messageEnd", "error"] = Field(
